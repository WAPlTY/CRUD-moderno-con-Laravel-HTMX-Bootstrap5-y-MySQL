<h1 class="text-center">
    <a class="float-start" title="Volver" 
       href="{{ route('home') }}"
       hx-get="{{ route('home') }}"
...
<form action="{{ route('myUpdate', $empleado->id) }}" method="POST" enctype="multipart/form-data"
          hx-put="{{ route('myUpdate', $empleado->id) }}"
          hx-target="#empleados-table tbody"
          hx-encoding="multipart/form-data"
          id="form-edit-empleado">
    @csrf
    @method("PUT")
...
<script>
    ...
    document.body.addEventListener('htmx:beforeRequest', function(evt) {
        if (evt.detail.elt.id === 'form-edit-empleado') {
    ...
    // Redirigir a home después de actualización exitosa
    document.body.addEventListener('htmx:afterSwap', function(evt) {
        if (evt.detail.target.id === 'empleados-table' && evt.detail.xhr.status === 200) {
            // Cargar la vista principal
            htmx.ajax('GET', '{{ route('home') }}', '#main-content');
        }
    });
</script>